export function calculateTwoNumbers(operation: string, x: number, y: number): number {
    switch (operation) {
        case 'add': 
            return x + y;
        case 'subtract': 
...
try {
    const result = calculateTwoNumbers('divide', 10, 0);
    console.log(result);
} catch (err) {
...
try {
    const result = calculateTwoNumbers('module', 10, 3);
    console.log(result);
} catch (err) {
